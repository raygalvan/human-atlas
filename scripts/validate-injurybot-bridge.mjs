...
assert.deepEqual(readInjuryBotHostConfig('?embed=injurybot&parentOrigin=https%3A%2F%2Finjury.bot&caseId=c&caseTitle=T&caseClient=Homer%20Cortez').initialCase,{id:'c',title:'T',client:'Homer Cortez'});
...
assert.equal(parseInjuryBotHostMessage({type:'injurybot:atlas:init',version:INJURYBOT_ATLAS_PROTOCOL,case:{id:'c',title:'T',client:'Homer Cortez',findings:[],activeReferenceGroups:[]}})?.case.client,'Homer Cortez');
