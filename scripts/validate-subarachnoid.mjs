...
for(const catalogue of ['atlas','inspection/detail']){
 ...
 for(const id of SAH.parents){
  ...
  const before=Buffer.from(g.getAttribute('position').array.buffer,g.getAttribute('position').array.byteOffset,g.getAttribute('position').array.byteLength).toString('base64'),overlay=subarachnoidGeometry(g,42),pos=overlay.getAttribute('position');assert(pos.count>300,id+' must have a genuine mesh patch');count+=pos.count/3;
  ...
  assert.equal(before,Buffer.from(g.getAttribute('position').array.buffer,g.getAttribute('position').array.byteOffset,g.getAttribute('position').array.byteLength).toString('base64'));assert(overlay.boundingBox.max.y-overlay.boundingBox.min.y>.003,'Follows actual folds, not a plane');
 }
