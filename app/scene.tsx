...
lastState=s;lastExtent=amount;dirty=true;}
   ...
   if(turnRemaining>0&&s.abrasionMode){const step=Math.min(turnRemaining,dt*.65);camera.position.sub(controls.target).applyAxisAngle(new T.Vector3(0,1,0),step).add(controls.target);turnRemaining-=step;controls.update();dirty=true;}el.dataset.abrasionTurning=String(turnRemaining>0);
