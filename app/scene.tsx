...
`+shader.fragmentShader;
  ...
  const syncDetail=()=>{
   ...
   atlas.parts.forEach((p,i)=>{const base=baseGeometry.get(i);if(!base)return;const g=s.isolate&&s.sourceDetail!==false&&s.selected.includes(p.id)&&approvedDetail.has(i)?detailGeometry.get(i)??base:base;if(activeGeometry.get(i)!==g){setActiveGeometry(i,g);changedChunks.add(p.chunk);}});
  ...
  const resize=()=>{focusKey='';layoutKey='';lastState=null;renderer.setPixelRatio(Math.min(devicePixelRatio,el.clientWidth<768||el.clientHeight<600?1.5:2));camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer.setSize(el.clientWidth,el.clientHeight);fit(latest.current.view,amount);};const observer=new ResizeObserver(resize);observer.observe(el);
  ...
  const up=(e:PointerEvent)=>{const validTap=tap.up(e.pointerId,e.clientX,e.clientY);if(!validTap||!ready)return;const rect=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-rect.left)/rect.width*2-1,-(e.clientY-rect.top)/rect.height*2+1);raycaster.setFromCamera(pointer,camera);let nearest=Infinity,found=-1;const hasSolid=atlas.parts.some((p,i)=>p.system!=='integumentary'&&data[i*4+3]>.5);pickers.forEach((mesh,i)=>{if(!mesh||data[i*4+3]<.5||(hasSolid&&atlas.parts[i].system==='integumentary'))return;worldBox.copy(bounds[i]).translate(mesh.position);if(!raycaster.ray.intersectBox(worldBox,hitPoint))return;const hits=raycaster.intersectObject(mesh,false);const hit=hits.find(h=>!isClipped(h.point.x-mesh.position.x,skullIds.has(atlas.parts[i].id),latest.current.skullReveal??0));if(hit&&hit.distance<nearest){nearest=hit.distance;found=i;}});if(found<0&&amount>.45)found=findTarget(e.clientX-rect.left,e.clientY-rect.top,e.pointerType==='touch'?24:16);if(found>=0){hover.hidden=true;select.current(atlas.parts[found].id);}};
  ...
  const animate=()=>{if(disposed)return;frame=requestAnimationFrame(animate);const elapsed=clock.getDelta(),dt=Math.min(elapsed,.05),s=latest.current;const detailKey=ready&&s.isolate&&s.sourceDetail!==false?s.selected.filter(id=>brainIds.has(id)||skullIds.has(id)||groups.rib.includes(id)).join(','):'';if(detailKey!==detailRequestKey){detailRequestKey=detailKey;syncDetail();if(detailKey)void requestDetail(detailKey.split(','));else reportDetail('base');}const changed=lastState?.abrasionMode!==s.abrasionMode||lastState?.abrasionVisible!==s.abrasionVisible||lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate||lastState?.selectionMode!==s.selectionMode||lastState?.hiddenParts!==s.hiddenParts||lastState?.skullReveal!==s.skullReveal||lastState?.registrationTest!==s.registrationTest;const moving=Math.abs(amount-s.explode)>.0001;if(moving){amount=T.MathUtils.damp(amount,s.explode,8,dt);dirty=true;}if(changed||moving||lastExtent<0){
    ...
    const skinMat=mats.get('integumentary')!;const opaque=!!s.abrasionMode;if(skinMat.transparent===opaque){skinMat.transparent=!opaque;skinMat.opacity=opaque?1:.1;skinMat.depthWrite=opaque;skinMat.needsUpdate=true;}
