...
import {inspectionGroups,type Inspection,type DetailStatus} from './inspection';
...
export function InspectionControls({atlas,state,detailStatus,onInspect,onChange,onRestore}:Props){
 ...
 return <details className="inspection-controls" open={mode?true:undefined}>
  ...
  {mode&&<div className="inspection-options">
   ...
   <div className="inspection-views" aria-label="Inspection directions">{([['front','Front'],['back','Rear'],['left','Left'],['right','Right'],['superior','Top'],['inferior','Base']] as [View,string][]).map(([view,label])=><Button key={view} variant="ghost" aria-label={`Inspect from ${label.toLowerCase()}`} onClick={()=>onChange({view,reset:state.reset+1,rotate:false})}>{label}</Button>)}</div>
   <p className="patient-orientation">Left and right always refer to the patient.</p>
   {developer&&<div className="registration-test"><strong>Developer test · synthetic</strong><p>Blue surface patch only. Not a Homer finding.</p><div className="inspection-option"><span>Registration patch</span><Switch aria-label="Synthetic registration patch" checked={!!state.registrationTest} onCheckedChange={value=>onChange({registrationTest:!!value})}/></div></div>}
